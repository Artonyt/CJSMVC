...
if (isset($_GET['delete_id'])) {
    ...
    if (!empty($delete_id)) {
        $query = "DELETE FROM materias WHERE ID_materia = :delete_id";
...
// Obtener y mostrar la lista de materias con nombres de asignaturas
$query = "SELECT materias.ID_materia, materias.Nombre_materia, asignaturas.Nombre_asignatura 
          FROM materias 
          INNER JOIN asignaturas ON materias.ID_asignatura = asignaturas.ID_asignatura";
...
<body>
    ...
    <section class="admin">
        ...
        <div class="crear-materia">
            <a href="/dashboard/cjs/views/administrador/materias/create.php" class="button boton-centrado">Crear Nueva Materia</a>
        </div>
                    ...
                    <tbody>
                        ...
                        if ($result->rowCount() > 0) {
                            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                echo "<td>";
                                $url_update = '/dashboard/cjs/views/administrador/materias/update.php?id=' . $row['ID_materia'];
                                echo "<a href='" . htmlspecialchars($url_update) . "' class='boton-modificar'><img src='../../../assets/editar.svg' alt='Editar'></a>";
                                $url_delete = htmlspecialchars($_SERVER["PHP_SELF"]) . "?delete_id=" . $row['ID_materia'];
                                echo "<a href='#' class='boton-eliminar' onclick=\"confirmarEliminar('" . $row['ID_materia'] . "')\"><img src='../../../assets/eliminar.svg' alt='Eliminar'></a>";
                                echo "</td>";
                                echo "</tr>";
                            }
                        } else {
    ...
    <script>
        ...
        function confirmarEliminar(id) {
            Swal.fire({
                title: '¿Estás seguro?',
                text: "Esta acción eliminará la materia.",
                icon: 'warning',
